...
class ConcurrentTradingWorker:
    ...
    def __init__(
        ...
        print(f"✅ Concurrent worker initialized")
    ...
    def _load_positions_from_db(self):
        ...
        try:
            ...
            print(f"📂 Loaded {len(positions)} open positions from database")
            ...
            if signal:
                ...
                if self.auto_trading:
                    ...
                    if position_id:
                        ...
                        print(f"  ✅ {pair}: {signal.signal} signal executed (Position: {position_id})")
                    else:
                        print(f"  ⚠️ {pair}: Could not execute signal")
                else:
                    print(f"  🎯 {pair}: {signal.signal} signal generated ({signal.confidence*100:.0f}% confidence)")
            else:
                decision_signal = decision.get('signal', 'HOLD')
                if decision_signal != 'HOLD':
                    print(f"  ⏸️ {pair}: {decision_signal} (confidence too low: {decision.get('confidence', 0)}%)")
    ...
    def analyze_all_pairs_concurrent(self) -> List[Dict]:
        ...
        """
        print(f"\n🔄 Analyzing {len(ForexConfig.ALL_PAIRS)} pairs concurrently...")
    ...
    def start(self):
        ...
        print("✅ Concurrent worker started (running every 60 seconds)")
